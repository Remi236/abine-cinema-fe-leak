...
function AppMovieMostView() {
  const [array, setArray] = useState([]);
  useEffect(() => {
    const MovieMostView = async () => {
      const response = await GetOrDelete('movies/most-view', 'GET');
      const json = await response.json();
      return json;
    };
    MovieMostView().then((json) => {
...
export default AppMovieMostView;
