...
>>>>>>> 182db25 (fix managerment movie & screening)
 ...
 const onChangPage = (pageIndexOfItems) => {
  ...
  const onAddSubmitting = errors => {
    const addCinnemaComplexsApi = async (valuesObject) => {
      const access_token = sessionStorage.getItem("access_token");
      const response = await PostOrPut("cinema-complexes", "POST", valuesObject, {
        Authorization: `Bearer ${access_token}`,
      });
      return response;
    }

    const form = document.forms["cinema_complex_add_form"];
    // validate
    if(errors && Object.keys(errors).length === 0) {
      ...
      addCinnemaComplexsApi(valuesObject).then((response) => {
        if(ERROR_CODES.includes(response.statusCode)) {
          alert(response.message);
          if(response.statusCode === "403") {
            window.location.href = "/403"
      ...
      <Card>
        <CardBody>
            <div className="card__title">
              <h5 className="bold-text">Data Cinema Complex</h5>
            </div>
              ...
              <div className="table__search d-flex align-items-center justify-content-between input-group-sm">
                ...
                <Fragment>
                  <CustomModal 
                    ...
                    message={<CinemaComplexForm iconSummit={iconSummit} onSubmitting={onAddSubmitting} action="add"/>}
                    color="primary"
                    header={true}
                    isOpen={isOpenModal}
...
export default CinemaComplexTable;